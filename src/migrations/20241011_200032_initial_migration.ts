...
export async function up({ payload, req }: MigrateUpArgs): Promise<void> {
  ...
  DO $$ BEGIN
   CREATE TYPE "public"."enum_orders_status" AS ENUM('pending', 'successful');
  EXCEPTION
   WHEN duplicate_object THEN null;
  END $$;
  ...
  CREATE TABLE IF NOT EXISTS "orders_fees" (
  	"_order" integer NOT NULL,
  	"_parent_id" uuid NOT NULL,
  	"id" varchar PRIMARY KEY NOT NULL,
  ...
  DO $$ BEGIN
   ALTER TABLE "orders_fees" ADD CONSTRAINT "orders_fees_parent_id_fk" FOREIGN KEY ("_parent_id") REFERENCES "public"."orders"("id") ON DELETE cascade ON UPDATE no action;
  EXCEPTION
   WHEN duplicate_object THEN null;
  END $$;
  ...
  CREATE INDEX IF NOT EXISTS "orders_fees_order_idx" ON "orders_fees" USING btree ("_order");
  CREATE INDEX IF NOT EXISTS "orders_fees_parent_id_idx" ON "orders_fees" USING btree ("_parent_id");
