...
export const getProductInfo = (product: Product) => {
  ...
  const model = typeof product.model === 'string' ? product.model : product.model.name

  const collaboration =
    typeof product.collaboration === 'string' ? product.collaboration : product.collaboration?.name
