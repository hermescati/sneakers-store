        ...
        <Drawer.Root handleOnly open={isOpen} onOpenChange={setIsOpen}>
            ...
            <Drawer.Portal>
                ...
                <Drawer.Content className="fixed inset-0 z-30 flex items-center justify-center outline-none">
                    <div ref={contentRef} className="relative px-6 pb-6 pt-5 mx-4 rounded-2xl border border-border bg-background shadow-lg">
                        <Drawer.Title className="font-semibold text-xl">Notify Me</Drawer.Title>
                        <IconButton
                            icon="tabler:x"
                            className="absolute top-3 right-3 text-xl md:text-2xl"
                            onClick={() => setIsOpen(false)} />
                        <Drawer.Description className="mt-1.5 text-md text-primary-700 leading-snug">Enter your email and size to be notified when it&apos;s back in stock.</Drawer.Description>
