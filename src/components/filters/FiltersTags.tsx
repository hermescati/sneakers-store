...
const FilterTags = ({
  ...
  const cleanedFilters = Object.entries(appliedFilters).filter(
      ...
      {cleanedFilters.map(([key, value]) => {
        ...
        if (key === 'brand' && Array.isArray(value)) {
          return value.map(v => (
            <Tag
              key={v}
              label={getLabelFromValue(v, brandOptions)}
              onRemove={() => handleTagRemove(key, value, v)}
            />
          ))
        }
        ...
        if (key === 'model' && Array.isArray(value)) {
          return value.map(v => (
            <Tag
              key={v}
              label={getLabelFromValue(v, modelOptions)}
              onRemove={() => handleTagRemove(key, value, v)}
            />
          ))
        }
        ...
        if (key === 'collaboration' && Array.isArray(value)) {
          return value.map(v => (
            <Tag
              key={v}
              label={getLabelFromValue(v, collabOptions)}
              onRemove={() => handleTagRemove(key, value, v)}
            />
          ))
        }
        ...
        if (key === 'category' && typeof value === 'string') {
          return (
            <Tag
              key={key}
              label={getLabelFromValue(value, categoryOptions) || value}
              onRemove={() => handleTagRemove(key, value)}
            />
          )
        }
